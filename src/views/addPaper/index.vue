<template>
  <div class="add-paper">
    <el-steps :active="active" finish-status="success" class="steps" >
      ...
      <!-- <el-step title="选择题目" /> -->
      <el-step title="游览题目" />
      <el-step :title="error ? '失败' : '成功'" :status="error ? 'error' : 'process'" />
    </el-steps>
    <div class="step-content">
      <OneStep v-if="active === 0" :clickNext="next" />
      <!-- <TwoStep v-if="active === 1" :clickNext="next" :clickPre="prev" /> -->
      <ThreeStep v-if="active === 1" :done="done" :clickPre="prev" :clickNext="next" />
      <FourStep v-if="active === 2" :clickPre="prev" :error="error" :goTestPaper="goTestPaper" />
    </div>
  </div>
</template>
